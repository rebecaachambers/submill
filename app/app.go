...
func (app *App) Initialize() error {
	...
	// Initialize embedded node file watcher (replaces external bash worker)
...
func (app *App) Run() {
	...
	// Start embedded file watcher (syncs output -> mihomo/nodes)
...
func (app *App) setTimer() {
	...
	if config.GlobalConfig.CronExpression != "" {
		...
		if err != nil {
			slog.Error(fmt.Sprintf("cron '%s' parse failed: %v, using interval",
...
func (app *App) triggerCheck() {
	...
	if app.ticker != nil {
		...
		if len(entries) > 0 {
			...
			slog.Info(fmt.Sprintf("Next check: %s", nextTime.Format("2006-01-02 15:04:05")))
		}
	}
	debug.FreeOSMemory()
}
